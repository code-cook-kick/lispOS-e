...
; Simplified approach avoiding lambda storage in data structures
...
; Returns: ('statute :id <id> :title <title> :weight <number> :props <props>)
...
(define statute.with-weight (lambda (s w)
  (list 'statute
        ':id (statute.id s)
        ':title (statute.title s)
        ':weight w
        ':props (statute.props s))))
