...
(define get-legitimate-children
  (lambda (ev)
    (if (> (length ev) 6)
        (nth ev 6)
        (list))))

(define get-illegitimate-children
  (lambda (ev)
    (if (> (length ev) 8)
        (nth ev 8)
        (list))))
...
(define calculate-inheritance-shares
  (lambda (ev)
...
(define calculate-intestate-shares
  (lambda (ev)
    (if (and
  (> (length (get-legitimate-children ev)) 0)
...
(define calculate-legitimate-only-shares
  (lambda (ev)
    (map2
      (lambda (child)
      (fact.make
        ...
        (list
        ':share
        (/ 1 (length (get-legitimate-children ev)))
        ':status
        'legitimate
        ':basis
...
(define calculate-illegitimate-only-shares
  (lambda (ev)
    (map2
      (lambda (child)
      (fact.make
        ...
        (list
        ':share
        (/ 1 (length (get-illegitimate-children ev)))
        ':status
        'illegitimate
        ':basis
        'intestate-succession)))
...
(define calculate-mixed-shares
  (lambda (ev)
    (append2
      (calculate-legitimate-mixed-shares ev)
      (calculate-illegitimate-mixed-shares ev))))

(define calculate-legitimate-mixed-shares
  (lambda (ev)
    (map2
      (lambda (child)
      (fact.make
...
(define calculate-illegitimate-mixed-shares
  (lambda (ev)
    (map2
      (lambda (child)
      (fact.make
...
(define statute.with-weight
  (lambda (s w)
    (list 'statute (statute.id s) (nth s 2) w (nth s 4))))

(define enhanced-inheritance.when
  (lambda (ev)
    #t))

(define enhanced-inheritance.then
  (lambda (ev)
    (calculate-inheritance-shares ev)))

(define enhanced-inheritance.eval
  (lambda (s ev)
    (if (enhanced-inheritance.when ev)
        (list
        (enhanced-inheritance.then ev)
...
(define ENHANCED-S774
  (statute.make
    'ENHANCED-S774
    "Dynamic Inheritance Calculator"
    (list)))
...
(define RESULT-LEGIT
  (registry.apply ENHANCED-REG EV-LEGIT-ONLY))
...
(define RESULT-ILLEGIT
  (registry.apply ENHANCED-REG EV-ILLEGIT-ONLY))

(define FACTS-ILLEGIT
  (first RESULT-ILLEGIT))

(print
  "Illegitimate only - Facts count:"
  (length FACTS-ILLEGIT))
...
(define EV-MIXED
  (event.make
    ...
    (list 'Maria 'Juan)
    ':illegitimate-children
    (list 'Ana 'Carlos 'Sofia)
    ':flags
    (list 'no-will))))

(define RESULT-MIXED
  (registry.apply ENHANCED-REG EV-MIXED))

(define FACTS-MIXED
  (first RESULT-MIXED))

(print
  "Mixed inheritance - Facts count:"
  (length FACTS-MIXED))
