...
(define calculate-intestate-shares
  (lambda (event)
    (let
      ((person (get-event-property event ':person))
      ...
      (children-total 0.75))
      (let
      ((child-share (/ children-total child-count)))
      (cons
      (make-fact
      'heir-share
      (list person spouse)
...
(define calculate-testate-shares
  (lambda (event)
    (let
      ...
      (if will
        (let
        ((bequests (get-event-property will ':bequests))
        ...
        (if bequests
          ...
          (make-fact
            ...
            (list
            ':share
            (get-event-property bequest ':share)
            ':basis
            'art-904-906
            ':heir-type
            'legatee)))
        ...
        (if residue
          (let
          ((residue-heirs (get-event-property residue ':heirs))
          (residue-share (get-event-property residue ':share)))
          (if (and residue-heirs (not (eq? (length residue-heirs) 0)))
            (let
            ((individual-share (/ residue-share (length residue-heirs))))
            (map
            (lambda (heir)
            (make-fact
              'heir-share
...
(define sum-shares
  (lambda (fact-list total)
    (if (eq? (length fact-list) 0)
        total
        (let
        ((f (first fact-list)) (remaining (rest fact-list)))
        (let
    ...
    (begin
      ...
      (if (eq? (length facts) 0)
          ...
          (map
            (lambda (f)
            (let
              ((args (get-event-property f ':args))
              (props (get-event-property f ':properties)))
              (let
              ((heir (first (rest args)))
              (share (get-event-property props ':share))
            ...
            facts)
          (let
            ((total (calculate-total-shares facts)))
            (print "  TOTAL SHARES: " total)
            (print
            "  MATHEMATICALLY CORRECT: "
            (and (>= total 0.99) (<= total 1.01))))))
      (print ""))))
...
(print
  "SCENARIO 1: Intestate Succession - ACTUAL COMPUTATION")
...
(define juan-results
  (calculate-intestate-shares juan-event))
...
(define rosa-results
  (calculate-testate-shares rosa-event))
...
(define antonio-results
  (calculate-intestate-shares antonio-event))
...
(define luis-will
  (list
    ':bequests
    (list (list ':legatee 'Carmen ':share 0.6))
    ':residue
    #f))
...
(define luis-testate-results
  (calculate-testate-shares luis-event))
...
(define luis-intestate-base
  (calculate-intestate-shares luis-intestate-event))
...
(define luis-intestate-scaled
  (map
    ...
    (let
      ...
      (props (get-event-property f ':properties)))
      (let
      ((original-share (get-event-property props ':share)))
      (make-fact
      'heir-share
      args
      (list
      ...
      (* original-share 0.4)
      ':basis
      'art-960-partial-intestacy
      ':heir-type
      'intestate-remainder)))))
    luis-intestate-base))

(define luis-combined-results
  (append luis-testate-results luis-intestate-scaled))

(display-results
  "Luis's Mixed Succession (Testate + Intestate)"
  luis-combined-results)
...
(print
  "✓ ACTUAL COMPUTATION: All results computed dynamically")
...
(print
  "🚀 System performs ACTUAL legal reasoning computation!")
