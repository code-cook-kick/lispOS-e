...
(define statute.make
  (lambda (id title when then props)
...
(print
  "Note: This demo uses simplified logic due to ASTNode symbol")
  ...
  (lambda (plist k v)
    (if (eq? (length plist) 0)
        ...
        (if (eq? (first plist) k)
          ...
          (cons
          (first plist)
          (cons (second plist) (plist-put-safe (rest (rest plist)) k v)))))))
...
(define safe-append
  (lambda (a b)
    (if (eq? (length a) 0)
        b
        (cons (first (ensure-list a)) (safe-append (rest a) b)))))
...
(define p-death
  (lambda (ev)
    (and (not (eq? ev null)) (eq? (event.type ev) 'death))))
...
(define p-no-will
  (lambda (ev)
    (if (eq? ev null)
        ...
        (and (not (safe-empty? fs)) (contains? fs 'no-will))))))
