...
;;; ===================================================================
;;; This demonstrates how to use the Etherney eLISP Legal Reasoning System
;;; for real-world Philippine succession scenarios

(print "=== PRACTICAL USAGE EXAMPLE: PHILIPPINE SUCCESSION LAW ===")
(print "")
...
(print "=========================================")
(print "Facts: Juan dies without a will, survived by 3 children")
(print "")
...
(define juan-death-event
  (list 'event ':type 'death
        ...
        ':jurisdiction 'PH
        ':flags (list 'no-will)
        ':legitimate-children (list 'Maria 'Pedro 'Ana)))
...
;; Expected result: Each child gets 1/3 share
...
(print "  Legal Basis: Article 979 - Children inherit equally")
(print "")

;;; -------------------------------------------------------------------
;;; SCENARIO 2: TESTATE SUCCESSION WITH WILL
;;; -------------------------------------------------------------------

(print "SCENARIO 2: Testate Succession with Will")
(print "========================================")
(print "Facts: Rosa dies with a valid will, specific bequests + residue")
(print "")

;; Create will structure
(define rosa-will
  (list ':id 'WILL-2024-001
        ':revoked #f
        ':bequests (list 
          (list ':id 'BEQUEST-1 ':legatee 'Carlos ':share 0.3 ':type 'specific ':condition #f)
          (list ':id 'BEQUEST-2 ':legatee 'Sofia ':share 0.2 ':type 'general ':condition #f))
...
(define rosa-death-event
  (list 'event ':type 'death
        ...
        ':jurisdiction 'PH
        ':will rosa-will
        ':compulsory-heirs (list 'Miguel 'Elena)))
...
(print "  Compulsory heirs: Miguel, Elena (legitime protection)")
(print "")
...
(print "  Total: 1.0 (complete estate)")
(print "  Legal Basis: Articles 904-906 (bequests), Article 906 (residue)")
(print "")

;;; -------------------------------------------------------------------
;;; SCENARIO 3: PARTIAL INTESTACY (MIXED SUCCESSION)
;;; -------------------------------------------------------------------

(print "SCENARIO 3: Partial Intestacy (Mixed Succession)")
(print "================================================")
(print "Facts: Luis dies with will covering only 60% of estate")
(print "")

;; Create partial will
(define luis-partial-will
  (list ':id 'WILL-2024-002
        ':revoked #f
        ':bequests (list 
          (list ':id 'BEQUEST-3 ':legatee 'Carmen ':share 0.6 ':type 'specific ':condition #f))
        ':residue #f))  ; No residue clause - creates partial intestacy
...
(define luis-death-event
  (list 'event ':type 'death
        ':person 'Luis
        ':jurisdiction 'PH
        ':will luis-partial-will
        ':legitimate-children (list 'Roberto 'Isabel)))
...
(print "  Legal Basis: Article 904-906 (bequest) + Article 960 (partial intestacy)")
(print "")

;;; -------------------------------------------------------------------
;;; SCENARIO 4: LEGITIME PROTECTION
;;; -------------------------------------------------------------------

(print "SCENARIO 4: Legitime Protection")
(print "===============================")
(print "Facts: Teresa tries to give 90% to non-compulsory heir")
(print "")

;; Create will that violates legitime
(define teresa-will
  (list ':id 'WILL-2024-003
        ':revoked #f
        ':bequests (list 
          (list ':id 'BEQUEST-4 ':legatee 'Charity ':share 0.9 ':type 'specific ':condition #f))
        ':residue #f))
...
(define teresa-death-event
  (list 'event ':type 'death
        ...
        ':jurisdiction 'PH
        ':will teresa-will
        ':compulsory-heirs (list 'Diego 'Lucia)))

(print "Event created:")
(print "  Deceased: Teresa")
(print "  Attempted bequest: 90% to Charity")
(print "  Compulsory heirs: Diego, Lucia")
(print "  Issue: Violates legitime (minimum 50% for compulsory heirs)")
(print "")
...
(print "  Charity: 0.50 share (reduced from 90% to respect legitime)")
(print "  Total: 1.0 (legitime + disposable portion)")
(print "  Legal Basis: Articles 886-906 (legitime protection)")
(print "")

;;; -------------------------------------------------------------------
;;; SCENARIO 5: SPOUSE AND CHILDREN SUCCESSION
;;; -------------------------------------------------------------------

(print "SCENARIO 5: Spouse and Children Succession")
(print "==========================================")
(print "Facts: Antonio dies intestate, survived by spouse and children")
(print "")
...
(define antonio-death-event
  (list 'event ':type 'death
        ...
        ':flags (list 'no-will)
        ':spouse 'Esperanza
        ':legitimate-children (list 'Fernando 'Gabriela 'Hector)))
...
(print "  Legal Basis: Article 996 (spouse 1/4, children 3/4)")
(print "")
...
(print "   (load \"src/lisp/statute-api-final-working.lisp\")")
...
(print "           ':will my-will-structure  ; or omit for intestate")
(print "           ':legitimate-children (list 'Child1 'Child2)")
(print "           ':spouse 'SpouseName))    ; if applicable")
(print "")
(print "3. APPLY THE LEGAL SYSTEM:")
(print "   ; For complete succession (testate + intestate)")
(print "   (define result (registry.apply PH-COMPLETE-SUCCESSION-REGISTRY my-event))")
(print "   (define facts (first result))")
(print "")
(print "   ; For testate only")
(print "   (define result (registry.apply PH-TESTATE-REGISTRY my-event))")
(print "")
(print "   ; For intestate only")
(print "   (define result (registry.apply PH-INTESTATE-REGISTRY my-event))")
(print "")
(print "4. EXAMINE THE RESULTS:")
(print "   (safe-map (lambda (fact)")
(print "               (print \"Heir:\" (first (rest (fact.args fact)))")
(print "                      \"Share:\" (fact.get fact ':share)")
(print "                      \"Basis:\" (fact.get fact ':basis)))")
(print "             facts)")
(print "")
(print "5. VALIDATE TOTAL SHARES:")
(print "   (define total (safe-fold")
(print "                   (lambda (acc fact)")
(print "                     (+ acc (fact.get fact ':share)))")
(print "                   0 facts))")
(print "   (print \"Total shares:\" total)  ; Should be ≈ 1.0")
(print "")
...
(print "🚀 Ready to process real Philippine succession cases!")
(print "")
(print "For more details, see:")
(print "- LISP_LEGAL_REASONING_COMPLIANCE_REPORT.md")
(print "- src/lisp/testate-succession-tests.lisp (400+ test cases)")
