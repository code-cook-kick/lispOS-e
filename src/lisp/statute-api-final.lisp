...
(define statute.eval
  (lambda (s ev)
    (if (statute.when-eval s ev)
        (list
        (statute.then-eval s ev)
        (statute.with-weight s (+ (statute.weight s) 1)))
        (list (list) s))))

(define statute.when-eval
  (lambda (s ev)
    ((statute.when s) ev)))

(define statute.then-eval
  (lambda (s ev)
    ((statute.then s) ev)))
...
(define registry.apply
  (lambda (registry ev)
    (if (eq? (length registry) 0)
        ...
        (list
        (append2
        (first (statute.eval (first registry) ev))
        (first (registry.apply (rest registry) ev)))
        (cons
        (second (statute.eval (first registry) ev))
...
(define when-intestate
  (lambda (ev)
    (if (eq? (event.type ev) 'death)
        (has-no-will? ev)
        #f)))

(define equal-split-facts
  (lambda (ev)
    (map2
      (lambda (heir)
      (fact.make
