; Etherney eLisp Legal Operating System - Working Test Suite
...
(define run-test (lambda (label expected actual)
  (if (eq? expected actual)
      (define passed-tests (+ passed-tests 1))
      (define failed-tests (+ failed-tests 1)))))

; Helper to increment total
(define inc-total (lambda (x)
...
(inc-total #t)
(run-test "event.valid? returns true" #t (event.valid? test-event))
(inc-total #t)
(run-test "event.type returns death" 'death (event.type test-event))
(inc-total #t)
(run-test "event.get person" 'Pedro (event.get test-event ':person))

; Happy Path Tests - Facts
(define test-fact (fact.make 'heir-share (list 'Pedro 'Maria) (list ':share 0.5 ':basis 'S774)))

(run-test "fact.valid? returns true" #t (fact.valid? test-fact))
(run-test "fact.pred returns heir-share" 'heir-share (fact.pred test-fact))
(run-test "fact.get share" 0.5 (fact.get test-fact ':share))
(run-test "fact.get basis" 'S774 (fact.get test-fact ':basis))

; Plist Tests
(define test-plist (list ':name 'Pedro ':age 45))
(run-test "plist-get name" 'Pedro (plist-get test-plist ':name))
(run-test "plist-get age" 45 (plist-get test-plist ':age))
(run-test "plist-get missing" #f (plist-get test-plist ':missing))

(define extended-plist (plist-put test-plist ':country 'Spain))
(run-test "plist-put country" 'Spain (plist-get extended-plist ':country))

; Edge Cases
(run-test "event missing key" #f (event.get test-event ':missing))
(run-test "fact missing key" #f (fact.get test-fact ':missing))
...
(run-test "duplicate keys last wins" 'Juan (plist-get dup-plist ':name))
